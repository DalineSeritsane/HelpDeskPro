...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const user = await User.create({
      ...
      password,
      role: normalizedEmail === 'admin@helpdesk.com' ? 'admin' : 'user'
    });
    res.json({ user });
  } catch (error) {
...
app.post('/api/tickets', async (req, res) => {
  try {
    ...
    const aiReply = description.toLowerCase().includes('support')
    ...
    const ticket = await Ticket.create({
      ...
      userEmail,
      aiReply,
      routedToAdmin: description.toLowerCase().includes('support')
    });
